...
Across approaches, results tended to converge on similar patterns. The preponderance of our results suggest that, in general, linear or ``close-to'' linear relationships with social desirability (across response options) may fairly well represent the plurarity of assessment item functions. Additionally, similar information may be available \emph{for particular items} through both the traditional and contemporary measurement approaches.

For example, the majority of the subjective functional form classifications presented in Figure \ref{fig:lastone} could reasonably be described as ``effectively'' linear. That is, the contributor(s) to non-linearity occur(s) in the extremes (e.g., ``most extreme'' being somewhat less desirable than ``moderately extreme'', however, the general pattern procedes along the Kuncel \& Tellegen continuum in a generally progressive fashion). This suggests that an addition to the Kuncel and Tellegen (2009) functional form specifications could be an explication of the \emph{location} of the non-monoticity (e.g., linearity is the expectation, deviations are differently meaningful depending on where they manifest -- perhaps they are \emph{not as important} in the extremes (for example, ``s'' and ``z'' Figure \ref{fig:lastone} specifications).

Although informative deviations from linear functions do exist, they are predominantly associated with \emph{moderately rated} Edwards items. Consultation of the 25 randomly sampled item functions (Figure \ref{fig:Figure2}) also reflects this location pattern of non-monotonicity.\footnote{A full list of all 300 item functions are available in this paper's online resources.} It is quite plausible that the \emph{most informatively meaningful} ``U'' or ``inverted U'' shaped functions, when they occur, are reflective of some ambiguous or contextually primed desirability, and that this ambiguity or contextual moderation results in ``middle ground'' evaluation via the Edwards method.

\section{Study 2}\label{study-2}

A consistent observation by experimental proctors across the two item rating procedures is that the Kuncel and Tellegen (2009) approach seems to be \emph{more cognitively taxing} for raters than is the Edwards (1953) rating task. Study Two therefore collected targeted ratings via computer, with estimates of difficulty operationalized as response latencies.
...
All ratings were gathered online via Qualtrics (2014), utilizing the Kuncel and Tellegen (2009) rating scale. The ratings were made across fifteen Study 1 items that exhibited archetypal functional forms: asymmetric, linear, and quasi-symmetric. Because response latencies were being recorded, two practice items were administered at the beginning of the procedure so participants would have process familiarity prior to substantive data collection.

\section{Results}\label{results-1}

Applying conservative data screens excluded 30 individuals with total survey durations less than 300 seconds (n=16) or total response latencies less than 20 seconds across the final 20 items (less than 1 second per averaged item response; n=14). These screens were applied based on observations from experiment proctors that several participants did not appear to be fully engaged in the rating task. Additionally, two participants self-disclosed that they responded ``not at all carefully'' and were excluded from analysis. Further screens via the \emph{careless} R package (Yentes \& Wilhelm, 2021) identified individuals with 10 or more consecutive non-differentiating responses (n=7) as well as intra-individual response variability estimates of two standard deviations below the mean (\emph{M}=0.88; n=5) and these individuals were also excluded from analysis (see, for example, Dunn et al., 2018; Marjanovic et al., 2015). For retained individuals, response latencies of ``0'' were removed, then any time below the 10th percentile and above 90th percentile, from the remaining times, were removed. Afterwards,
...
Mean latencies were investigated at four different levels: item (e.g., worry about things), response category (e.g., extremely high), item type (e.g., linear), and item polarity (e.g., positive versus negatively worded items).
